...
/** Registers the "Flip Finder" (`/saibonflips`) settings section — one enable toggle per [FlipFinder] strategy plus the shared scan interval and alert thresholds. Distinct from `FlipFinderSettings`, which still governs `/saibonah`'s narrower sales-history engine specifically. */
